...
public class RedisJsonDAO {
	...
	//keyset is stored in a redis set, while key-value pairs are stored in a redis hashmap
	...
	public void persist(Set<? extends Object> set, String name) {
		...
		setOps.add("JSONStorage:set:name:"+name, set.stream().map(gson::toJson).collect(Collectors.toList()).toArray(new String[0]));
	}
